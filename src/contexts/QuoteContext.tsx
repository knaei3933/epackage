...
type QuoteAction =
  ...
  | { type: 'SET_QUANTITY_OPTIONS'; payload: { quantities?: number[]; quantity?: number; isUVPrinting: boolean; printingType?: 'digital' | 'gravure'; printingColors?: number; doubleSided?: boolean } }
...
function quoteReducer(state: QuoteState, action: QuoteAction): QuoteState {
  switch (action.type) {
    ...
    case 'SET_QUANTITY_OPTIONS':
      return {
        ...state,
        quantities: action.payload.quantities !== undefined ? action.payload.quantities : state.quantities,
        quantity: action.payload.quantity !== undefined ? action.payload.quantity : state.quantity,
...
interface QuoteContextType {
  ...
  updateBasicSpecs: (specs: Partial<QuoteState>) => void;
  updateQuantityOptions: (options: Partial<QuoteState>) => void;
  ...
  canAddPostProcessingOption: () => boolean;
  clearPostProcessingValidationError: () => void;
  getPostProcessingLimitStatus: () => PostProcessingLimitState;
}
...
export function QuoteProvider({ children }: QuoteProviderProps) {
  ...
  // Action helpers
  const updateBasicSpecs = (specs: Partial<QuoteState>) => {
  ...
  const updateQuantityOptions = (options: Partial<QuoteState>) => {
    dispatch({
      type: 'SET_QUANTITY_OPTIONS',
      payload: {
        quantity: options.quantity !== undefined ? options.quantity : state.quantity,
  ...
  const addPostProcessingOption = (
    ...
    return !state.postProcessingValidationError;
  };

  const removePostProcessingOption = (optionId: string) => {
    dispatch({
  ...
  const replacePostProcessingOption = (oldOptionId: string, newOptionId: string) => {
    dispatch({
  ...
  const canAddPostProcessingOption = (): boolean => {
    return !isSelectionLimitReached(state.postProcessingOptions.length);
  };

  const clearPostProcessingValidationError = () => {
    dispatch({ type: 'CLEAR_POST_PROCESSING_VALIDATION_ERROR' });
  };

  const getPostProcessingLimitStatus = (): PostProcessingLimitState => {
    return {
  ...
  const isStepComplete = (step: string): boolean => {
    switch (step) {
      case 'specs':
        ...
        // Check if material has thickness options
  ...
  // Step summary for display in subsequent steps
