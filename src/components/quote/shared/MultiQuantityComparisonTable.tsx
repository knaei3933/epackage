...
export default function MultiQuantityComparisonTable({
  ...
  // Sort quotes based on configuration
  ...
  const handleSort = (key: keyof QuantityQuote) => {
    setSortConfig(prev => ({
      key,
  ...
  const toggleRowExpansion = (quantity: number) => {
  ...
  const getBestValueIndicator = (quantity: number) => {
    if (!comparison) return null;
    return comparison.bestValue.quantity === quantity;
  };

  const getPriceTrendIcon = (quantity: number) => {
  ...
  const getDiscountBadgeColor = (discountRate: number) => {
    ...
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        <div className="flex items-center justify-center py-12">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-navy-600 mr-4"></div>
            ...
            {comparison && (
              <>
                <div className="flex items-center">
                  ...
                  <span>最適価値: {comparison.bestValue.quantity.toLocaleString()}個</span>
                </div>
        ...
        {sortedQuotes.map((quote, index) => (
          <motion.div
            ...
            className={`border rounded-lg p-4 cursor-pointer transition-all ${
              selectedQuantity === quote.quantity
                ? 'border-green-500 bg-green-50 shadow-md'
                : getBestValueIndicator(quote.quantity)
                ? 'border-yellow-400 bg-yellow-50 hover:shadow-md'
                : 'border-gray-200 hover:border-navy-300 hover:shadow-sm'
            }`}
            onClick={() => onQuantitySelect(quote.quantity)}
          >
            {/* Quantity Header */}
            <div className="flex items-center justify-between mb-3">
              ...
              {getPriceTrendIcon(quote.quantity)}
            </div>
      ...
      <div className="hidden lg:block overflow-x-auto">
        <table className="w-full">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {sortedQuotes.map((quote, index) => {
              const isBestValue = getBestValueIndicator(quote.quantity);
              const isSelected = selectedQuantity === quote.quantity;
              const isExpanded = expandedRows.has(quote.quantity);

              return (
                <React.Fragment key={quote.quantity}>
                  <motion.tr
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
