 ...
 *
 * 参照元（全て同一仕様）:
 *   - src/lib/pricing/core/constants.ts   : MANUFACTURER_MARGIN=0.4, DUTY_RATE=0.05
 *   - src/lib/pricing/strategies/base-strategy.ts:227 : manufacturerPrice = baseCost * 1.4
...
/** 製造者マージン率 40%（PRICING_CONSTANTS.MANUFACTURER_MARGIN と一致） */
export const MANUFACTURER_MARGIN_RATE = 0.4;
/** 関税率 5%（PRICING_CONSTANTS.DUTY_RATE と一致） */
export const DUTY_RATE = 0.05;

/**
 * 原価から製造者マージンを計算する。
 * manufacturingMargin = baseCost * MANUFACTURER_MARGIN_RATE(0.4)
 */
 ...
 *
 * @param baseCost 原価（売価を含まないこと。売価を渡すと約40%乖離する）
 * @param manufacturingMargin 製造者マージン。省略時は baseCost * 0.4 を使用
