...
const FILENAME_PATTERN = /^([^_]+)_(入稿データ|校正データ)_([A-Z0-9\-]+)_(\d{8})(?:_R(\d+))?$/;
 ...
 */
export function parseCustomerFilename(filename: string): ParsedFilename | null {
  ...
  if (match) {
    const [, language, marker, orderNumber, date, revisionStr] = match;

    return {
      language,
      orderNumber,
      date,
      type: marker === CORRECTION_MARKER || revisionStr ? 'correction' : 'submission',
 ...
 */
export function generateCorrectionFilename(
  originalFilename: string,
  ...
  const fallbackOrderNumber = orderNumber || parsed?.orderNumber || 'unknown';
 ...
 */
export function compareRevisionFilenames(filename1: string, filename2: string): number {
  ...
  if (rev1 < rev2) return -1; // filename2 is newer
  if (rev1 > rev2) return 1;  // filename1 is newer
  return 0;
}
