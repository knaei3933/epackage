 ...
 * | グラビア | 原価ベース(固定費除算) | 銅版費・セットアップ費を列数除算 → 15〜55% に漸近      |
 * | ロール   | 別系（現状維持）   | noop・既存 2-7列 40-64% カーブ（UnifiedSKUQuantityStep）  |
 *
 * 顧客には統一率（2列15% / 3列30% / 4列40% / 5列45% / 6列50% / 7列55%）を提示するが、
 * 内部計算は系統別に最適化してマージン逆転を回避する。
 *
 * 2026-06-28 改定（C2）: 4列打ち切りを解除し 5/6/7 列を追加。
 *   列数 = floor(印刷方式最大幅 ÷ 1列フィルム幅)（デジタル740mm / グラビア1100mm）。
 *   物理上限は GRAVURE_CONSTANTS.MAX_COLUMN_COUNT(=7)。
 *   納品形態別の更なる制約: パウチ袋=2列まで / ロール=最大7列
 *   （getAvailableColumnCounts 側、または呼び出し元で絞り込み）。
 */

/** 統一表示用の多列割引率（2列15% / 3列30% / 4列40% / 5列45% / 6列50% / 7列55%） */
 ...
 * @param columnCount 列数（1=単列・2〜7=多列）
