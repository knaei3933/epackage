 ...
 *
 * 判断対応:
 * - 判断3 : 製造者マージン 30% 確定（2026-07-09 改定 40%→30%・constants/DB/計算結果で一貫）
...
describe('価格計算ゴールデン値テスト（Phase 4 - 判断1(b)/2/3/9 回帰ガード）', () => {
  ...
  describe('定数値の固定（判断3/9/1(b)）', () => {
    it('MANUFACTURER_MARGIN = 0.3（判断3・製造者マージン30%確定・2026-07-09 改定 40%→30%）', () => {
      expect(PRICING_CONSTANTS.MANUFACTURER_MARGIN).toBe(0.3)
    })
