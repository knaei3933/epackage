 ...
 * B2B File Upload API
 *
 * B2B 파일 업로드 API 엔드포인트
 * - POST: AI/PDF 파일 업로드 및 저장
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    perfMonitor.trackQuery(`POST /api/b2b/files/upload`, duration);
  }
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    perfMonitor.trackQuery(`GET /api/b2b/files/upload`, duration);
  }
}
