 ...
 *
 * Routes:
 * - GET /api/b2b/quotations/[id]/export - 見積もりデータ取得
 * - POST /api/b2b/quotations/[id]/export - ファイル生成
 */
...
async function uploadToStorage(
  ...
  try {
    ...
    const supabase = createAuthenticatedServiceClient({
      ...
      userId,
      route: '/api/b2b/quotations/[id]/export',
    });
  ...
  try {
    ...
    if (format === 'excel') {
      ...
      if (!validation.isValid) {
        return NextResponse.json(
          {
            error: 'Invalid quotation data for PDF generation',
            code: 'INVALID_DATA',
    ...
    if (body.saveToStorage !== false) {
      ...
      const path = `${user.id}/${quotation.quotation_number}/${Date.now()}_${fileName}`;
      ...
      const uploadResult = await uploadToStorage(
        ...
        fileBuffer,
        contentType,
        user.id // Pass authenticated user ID for audit logging
      );
    ...
    const response: ExportResponse = {
      ...
      data: {
        ...
        format,
        downloadUrl: storagePath || `/api/b2b/quotations/${quotationId}/export/download?format=${format}`,
        storagePath,
        emailSent,
