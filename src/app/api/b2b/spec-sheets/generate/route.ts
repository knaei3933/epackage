 ...
 *
 * 주문 데이터로부터 사양서 생성 API
 * - AI 추출 데이터와 한국 파트너 수정사항 반영
 * - PDF 생성 및 데이터베이스 저장
 * - 고객 승인을 위한 준비
 *
...
export async function POST(request: NextRequest) {
  try {
    ...
    const supabaseAdmin = createAuthenticatedServiceClient({
      ...
      userId: user.id,
      route: '/api/b2b/spec-sheets/generate',
    });
    ...
    if (existingSpec) {
      ...
      const { data: newSpec, error: insertError } = await supabaseAdmin
        ...
        .insert({
          ...
          title: `사양서 - ${order.order_number}`,
          description: `${order.product_name || 'Custom Product'} 사양서`,
          specifications: specSheetData as any,
          pdf_url: publicUrl,
          status: 'pending_review',
          created_by: user.id,
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        pdfUrl: publicUrl,
        status: 'pending_review',
        message: '사양서가 생성되었습니다. 고객 승인을 기다리고 있습니다.',
      },
    });
  } catch (error: any) {
