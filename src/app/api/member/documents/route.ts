/**
 * Member Documents API (Integrated from /api/customer/documents)
 * GET /api/member/documents - List member's downloadable documents
 * POST /api/member/documents/log - Log document access
 *
 * Provides complete document data for:
 * - Portal pages (migrated to /admin/customers)
 * - Member pages
 *
 * Features:
 * - Quotations with PDFs
 * - Contracts
 * - Design files (AI)
 * - Type filtering
 * - Pagination
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Member Documents API error:', error);
    return NextResponse.json(
