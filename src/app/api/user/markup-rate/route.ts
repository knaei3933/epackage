...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  if (error) {
    return NextResponse.json({ success: true, data: { markupRate: 0.5, markupRateNote: null } });
  }
  return NextResponse.json({ success: true, data: { markupRate: data?.markup_rate ?? 0.5, markupRateNote: data?.markup_rate_note ?? null } });
}
