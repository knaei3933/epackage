...
export const POST = withMemberAuth<any>(
  async (request) => {
    try {
      ...
      if (
        body.service_type &&
        !Object.values(ShippingServiceType).includes(body.service_type)
      ) {
      ...
      const shipmentNumber = `SHP-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}-${Math.floor(
        Math.random() * 10000,
      )
        .toString()
        .padStart(4, '0')}`;
      ...
      const packageDetails = {
        ...
        special_handling:
          body.delivery_time_slot !== DeliveryTimeSlot.NONE
            ? [body.delivery_time_slot]
            : [],
      };
      ...
      const trackingNumber =
        (body as any).tracking_number || carrierResponse.trackingNumber;
      ...
      const { data: shipment, error: shipmentError } = await (supabase as any)
        ...
        .insert({
          ...
          carrier_code:
            carrierCodeMap[body.carrier] ||
            body.carrier.toUpperCase().slice(0, 3),
          service_level:
            serviceLevelMap[
              body.service_type || ShippingServiceType.TAKKYUBIN
            ] || 'STANDARD',
      ...
      if (shipmentError || !shipment) {
        console.error(
          '[Shipment] DB Error Details:',
          JSON.stringify(shipmentError, null, 2),
        );
        throw new ShipmentError(
      ...
      await (supabase as any).from('shipment_tracking_events').insert({
        ...
        location:
          DEFAULT_SENDER_ADDRESS.city +
          ', ' +
          DEFAULT_SENDER_ADDRESS.prefecture,
...
export const GET = withMemberAuth<any>(
  async (request) => {
    try {
      ...
      const readyOrders =
        orders?.filter(
          (order: any) => !order.shipments || order.shipments.length === 0,
        ) || [];
