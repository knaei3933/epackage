  ...
  async (_request, _auth, context) => {
    try {
      ...
      const trackingInfo = await getTrackingStatus(
        ...
        shipmentTyped.tracking_number,
      );

      // Check if status has changed
      const statusChanged =
        trackingInfo.current_status !== shipmentTyped.status;
      ...
      if (
        trackingInfo.current_status === ShipmentStatus.DELIVERED &&
        !shipmentTyped.delivered_at
      ) {
        updateData.delivered_at = new Date().toISOString();
      }

      const { data: updatedShipment, error: updateError } = await (
        supabase as any
      )
        .from('shipments')
        .update(updateData)
