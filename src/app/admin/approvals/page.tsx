 ...
 *
 * Admin approval page for new member registrations
 * - List pending member registrations
 * - Approve/Reject buttons
 * - Show member details (company, contact info)
...
export default function AdminApprovalsPage() {
  ...
  const { data, error, isLoading, mutate: refetch } = useSWR<ListPendingMembersResponse>(
      ...
      <div className="min-h-screen bg-gray-50 p-6 flex items-center justify-center">
        <Card className="max-w-md w-full">
          <CardContent className="pt-6">
            <EmptyState
              title="エラーが発生しました"
              description="承認待ち会員の読み込みに失敗しました。"
              action={{
                label: '再読み込み',
                onClick: () => refetch(),
              }}
            ...
            {pendingMembers.map((member) => (
              <Card key={member.id} className="overflow-hidden">
                ...
                <CardContent className="pt-6 space-y-4">
                  ...
                  {/* Corporate Number */}
                  {member.corporate_number && (
                    <div className="flex justify-between items-center py-2 border-b">
                      <span className="text-sm text-gray-500">法人番号</span>
                      <span className="text-sm font-medium font-mono">{member.corporate_number}</span>
                    </div>
                  )}

                  {/* Phone */}
                  {(member.corporate_phone || member.personal_phone) && (
                    <div className="flex justify-between items-center py-2 border-b">
                      <span className="text-sm text-gray-500">電話番号</span>
                      <span className="text-sm font-medium">
                        {formatPhoneNumber(member.corporate_phone || member.personal_phone)}
                      </span>
                    </div>
                  )}
                  ...
                  {(member.prefecture || member.city || member.street) && (
                    <div className="flex justify-between items-start py-2 border-b">
                      ...
                      <span className="text-sm font-medium text-right max-w-xs">
                        {[member.prefecture, member.city, member.street, member.building]
                          .filter(Boolean)
                          .join('')}
                      </span>
                    </div>
