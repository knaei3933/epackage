...
import { DocumentDownloadCard } from '@/components/portal/DocumentDownloadCard';
...
async function getOrderDetail(orderId: string) {
  ...
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/customer/orders/${orderId}`,
    {
      headers: {
  ...
  return (
    <div className="space-y-6">
      ...
      <nav className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
        <Link href="/portal" className="hover:text-slate-900 dark:hover:text-white">
          ポータル
        </Link>
        <span>/</span>
        <Link href="/portal/orders" className="hover:text-slate-900 dark:hover:text-white">
          注文一覧
        </Link>
        <span>/</span>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          {order.can_request_changes && (
            <Link
              href={`/portal/orders/${order.id}/request-changes`}
