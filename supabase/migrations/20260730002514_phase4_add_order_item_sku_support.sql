...
-- 注意: design_revisions の既存 SELECT ポリシーは 'Consolidated design_revisions
-- select policy'（ADMIN/order_id/quotation_id を包括）。本ポリシーは PERMISSIVE
-- なので PostgreSQL は OR 結合し、既存アクセス（ADMIN/quotation_id 経由）を
-- 一切狭めず、order_item_id 経由の SELECT のみを追加許可する。
-- =====================================================
