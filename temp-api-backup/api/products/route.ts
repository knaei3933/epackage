...
// Product categories - Package-Lab actual products
// Ordered according to requirements: 1.3방실파우치 2.스탠드파우치 3.가젯파우치 4.스파우트파우치 5.롤펄
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fallback: Return mock data based on design specification
    const mockProducts = getMockProducts(category, locale)
...
function getMockProducts(categoryFilter?: string | null, locale: string = 'ja') {
  const allProducts = [
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { action, productId, specifications } = body
