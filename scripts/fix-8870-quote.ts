...
async function main() {
  console.log('=== QT20260331-8870 재계산 ===\n');
  ...
  // 현재 저장된 값 확인
  const currentWeights = specs.film_cost_details?.materialLayerDetails || [];
  console.log('현재 저장된 중량 (오차 있음):');
  for (const layer of currentWeights) {
    console.log(`  ${layer.nameJa} ${layer.thicknessMicron}μm: ${layer.weightKg}kg`);
  }

  // Recalculate with correct filmLayers
  const params = {
