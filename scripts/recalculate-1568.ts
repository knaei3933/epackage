...
async function recalculateQT202604011568() {
  console.log('=== QT20260401-1568 再計算 ===\n');
  ...
  console.log('【再計算結果】');
  console.log('  pouchProcessingCost (breakdown):', result.breakdown?.processing || result.breakdown?.pouchProcessingCost);
  console.log('  pouchProcessingCost (filmCostDetails):', result.filmCostDetails?.pouchProcessingCost);
  ...
  console.log('  film_cost_details.pouchProcessingCost:', filmCostDetails?.pouchProcessingCost);
  console.log('');
  ...
  if (updateError) {
    ...
    console.log('');
    console.log('次の手順:');
    console.log('1. ブラウザで管理者ページをリロード');
