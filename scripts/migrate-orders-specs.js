/**
 * Order Specifications Migration Script
 *
 * 古い注文データの specifications に欠けているフィールドを補完します
...
async function migrateOrders() {
  ...
  console.log('Order Specifications Migration');
  console.log('='.repeat(60));
  console.log('');

  // すべての注文アイテムを取得
  console.log('1. Fetching order items...');
  ...
  if (fetchError) {
    console.error('ERROR: Failed to fetch order items:', fetchError.message);
    process.exit(1);
  }
  ...
  for (const item of orderItems) {
    ...
    if (updateError) {
      ...
      console.log(`   Updated item ${item.id} (${item.order_id})`);
      updatedCount++;
    }
  }
