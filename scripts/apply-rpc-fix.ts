/**
 * Apply RPC Fix via PostgreSQL
 *
 * This script uses the PostgreSQL connection string to execute DDL
 * and fix the RPC function schema issue.
 */
...
-- Create new function with explicit schema qualification and SET search_path
...
GRANT EXECUTE ON FUNCTION create_sample_request_transaction TO anon;
    ...
    console.log('✓ RPC function recreated with SET search_path = public\n');
    ...
    const testResult = await client.query(`
      SELECT * FROM create_sample_request_transaction(
        ...
        'Test note',
        '[{"productName": "Test Product", "productCategory": "other", "quantity": 1}]'::jsonb
      )
    `);
    ...
    if (testResult.rows[0].success) {
      ...
      console.log('\n✗ RPC function test failed:', testResult.rows[0].error_message);
    }
