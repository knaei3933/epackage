...
### 2026-02-18: 販売マージン率（default_markup_rate）検証完了
**目的**: 販売マージン率がDB設定と正しく連携しているか検証

**検証結果 (2026-02-18 完了)**:
✅ **default_markup_rateの変更が価格に正しく反映されることを確認**

| default_markup_rate | 単価 | 検証日時 |
|---------------------|------|----------|
| 0.2 (20%) | ¥204/個 | 2026-02-18 |
| 0.5 (50%) | ¥254/個 | 2026-02-18 |

**計算の妥当性確認**:
- 小計が約¥170と仮定
- 20%適用: 170 × 1.2 = ¥204 ✅
- 50%適用: 170 × 1.5 = ¥255 ≈ ¥254 ✅

**検証手順**:
1. 管理画面でdefault_markup_rateを0.2から0.5に変更
2. 見積もりシミュレーターで計算 → ¥254/個 ✅
3. 価格が正しく変動することを確認

**結論**: default_markup_rate（販売マージン率）のDB設定が正しく価格計算に反映されている

### 2026-02-18: calculateSKUCostのasync/await修正
**問題**: Vercelビルドエラー - `await isn't allowed in non-async function`

**根本原因**:
- `calculateSKUCost`メソッド内の`map`コールバック関数で`await`を使用していた
- しかし、`map`のコールバック関数が`async`として宣言されていなかった
- また、`Promise.all`で待機していなかった

**修正内容**:
- `skuQuantities.map((quantity, index) => {` → `await Promise.all(skuQuantities.map(async (quantity, index) => {`
- 閉じ括弧も`});` →`}));`に修正

**コミット**: `eb3c556` - fix: calculateSKUCostのmapコールバックをasyncに修正
