...
export class DatabaseVerifier {
  ...
  private beforeState: DatabaseState = { timestamp: '', tables: {} };
   ...
   * 테스트 전 상태 기록
   */
  async captureBeforeState(): Promise<DatabaseState> {
   ...
   * 테스트 후 상태 기록
   */
  async captureAfterState(): Promise<DatabaseState> {
    const timestamp = new Date().toISOString();
    ...
    const afterState: DatabaseState = {
      timestamp,
      tables: {
        quotations: quotationsCount,
        orders: ordersCount,
   ...
   */
  async executeSql(sql: string): Promise<{ data: any[] | null; error: string | null; rowsAffected?: number }> {
    try {
   ...
   * INSERT 쿼리 실행
   */
  private async executeInsertQuery(sql: string): Promise<{ data: any[] | null; error: string | null; rowsAffected?: number }> {
    try {
      // 간단한 INSERT 쿼리 파싱
   ...
   * UPDATE 쿼리 실행
   */
  private async executeUpdateQuery(sql: string): Promise<{ data: any[] | null; error: string | null; rowsAffected?: number }> {
    try {
      // 간단한 UPDATE 쿼리 파싱
      ...
      if (updateMatch) {
        ...
        // WHERE 절 파싱 (간단한 경우만)
        let query = this.client.from(tableName).update(setData as never);
   ...
   * DELETE 쿼리 실행
   */
  private async executeDeleteQuery(sql: string): Promise<{ data: any[] | null; error: string | null; rowsAffected?: number }> {
    try {
      // 간단한 DELETE 쿼리 파싱
   ...
   */
  private async executeSelectQuery(sql: string): Promise<{ data: any[] | null; error: string | null }> {
    try {
   ...
   * 검증 쿼리 실행
   */
  async verifyQueries(queries: VerificationQuery[]): Promise<VerificationQuery[]> {
    const results: VerificationQuery[] = [];
   ...
   * 간단한 SELECT 쿼리만 지원
   */
  private async tryParseAndExecuteQuery(sql: string): Promise<any> {
    try {
   ...
   * 예상값 검증
   */
  private verifyExpected(expected: any, actual: any): boolean {
  ...
  compareStates(before: DatabaseState, after: DatabaseState): Record<string, number> {
    ...
    for (const [table, beforeCount] of Object.entries(before.tables)) {
      const afterCount = after.tables[table as keyof typeof after.tables];
   ...
   * 특정 레코드 상세 검증
   */
  async verifyRecord(tableName: string, filters: Record<string, any>): Promise<any> {
    try {
   ...
   * 최신 레코드 확인
   */
  async getLatestRecord(tableName: string): Promise<any> {
    try {
      const { data, error } = await this.client
        .from(tableName)
        .select('*')
