...
import { loadScenarios, ParsedScenario } from './scenario-parser.js';
import { PlaywrightExecutor } from './playwright-executor.js';
import { DatabaseVerifier, VerificationQuery } from './database-verifier.js';
import { MarkdownReporter } from './reporters/markdown.js';
import { JsonReporter, TestSummary } from './reporters/summary.js';
...
export class TestRunner {
   ...
   */
  private getRequiredAuthType(scenarioKey: string): 'admin' | 'member' | null {
   ...
   * 단일 시나리오 실행
   */
  async runScenario(scenarioKey: string, scenario: ParsedScenario): Promise<any> {
    console.log(`\n${'='.repeat(60)}`);
    ...
    if (scenario.databaseQueries.length > 0 && !this.options.skipDatabase) {
      ...
      const queries: VerificationQuery[] = scenario.databaseQueries.map(query => ({
    ...
    // 개별 리포트 생성
    const markdown = this.markdownReporter.generateScenarioReport(
      scenarioResult,
      beforeState,
      afterState
    );
   ...
   * 특정 카테고리만 실행
   */
  async runCategory(category: 'homepage' | 'member' | 'admin' | 'integration'): Promise<TestRunResult> {
    const allScenarios = loadScenarios(this.options.baseDir);
...
export async function main() {
  ...
  if (category) {
    await runner.runCategory(category as any);
  } else {
    await runner.runAll();
  }
}
...
const isDirectExecution = process.argv[1] && process.argv[1].includes('index.ts');
