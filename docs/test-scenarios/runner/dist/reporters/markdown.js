...
export class MarkdownReporter {
     ...
     * 시나리오 결과를 마크다운으로 변환
     */
    generateScenarioReport(result, beforeState, afterState) {
        const lines = [];
        // 헤더
        ...
        if (beforeState && afterState) {
            ...
            for (const [table, afterCount] of Object.entries(afterState.tables)) {
                const beforeCount = beforeState.tables[table] || 0;
    ...
    generateSummaryReport(results, options = {}) {
        ...
        lines.push('');
        // 실패한 단계 목록
        const failedSteps = results.flatMap(r => r.results.filter(s => s.status === 'failed').map(s => ({ scenario: r.scenario, step: s })));
