...
export class PlaywrightExecutor {
     ...
     * 要素を見つける（複数の戦略を試行）
     */
    async findElement(selectorDescription) {
        if (!this.page)
                ...
                if (await this.isElementVisible(elem)) {
                    ...
                    try {
                        ...
                        if (await prevSibling.count() > 0) {
                            const prevText = await prevSibling.evaluate((el) => {
                    ...
                    try {
                        ...
                        for (const sibling of allPrevSiblings) {
                            const siblingText = await sibling.evaluate((el) => {
                                const textNodes = Array.from(el.childNodes)
                    ...
                    try {
                        ...
                        const parentText = await parent.evaluate((el) => {
                    ...
                    try {
                        const parent = elem.locator('xpath=..');
                        const allText = await parent.evaluate((el) => el.textContent || '');
    ...
    async executeStep(scenario, step, action, element, params) {
                ...
                case 'type':
                    ...
                    if (element && params?.text) {
                        ...
                        if (foundElement) {
                            ...
                            const tagName = await foundElement.evaluate((el) => el.tagName.toLowerCase());
                            if (tagName === 'select') {
